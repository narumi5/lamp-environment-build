...
3. VirtualBoxを利用してUbuntuをインストール（M1 macのためARM版をインストール）
...
6. `chronyc  sources`でNTPサーバーを確認する
7. `sudo systemctl restart chrony` で変更内容を反映する
...
2. ubuntuのファイアウォールポートの番号22番を許可する（macのターミナルで入るため）

`sudo ufw allow 22` 

3. ubuntuのファイアウォールポートの番号80番を許可する（http通信を行うため）
...
4. ブラウザが動いているのがmacのためmac側のhostsの設定変更
...
6. http://morii-hbtask.local/phpinfo.php にアクセスして表示確認
...
3. `sudo cat /var/www/firstname/mysql.php` で内容確認
...
4. `sudo tail -n 50 /var/log/apache2/error.log` でエラーのログ確認
...
5. `sudo apt install php7.4-mysql` でインストール
...
8. `narumi-hbtask.local/mysql.php`にアクセスして確認
...
10. `http://morii-hbtask.local/blog`で投稿した記事を閲覧できることを確認
